$(function() {
    ...
    editField.prototype = new jsGrid.Field({
        ...
        itemTemplate: function (value, row) {
            return $('<button type="button" class="btn btn-default btn-sm" data-toggle="modal" data-target="#editUserModal" data-uid="' + row["id"] + '" data-name="' + row["name"] + '" data-groupid="' + row["group_id"] + '" data-group="' + row["group"] + '" onclick="editUser();">编辑用户</button>');
        }
    });
